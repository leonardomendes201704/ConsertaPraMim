# PROVIDER_APP_WEB

Status: Done

## Objetivo da trilha

Construir o app do prestador com contratos mobile dedicados, mantendo isolamento dos portais Web para evitar regressao cruzada.

## Epics

- `EPIC-001` - App do prestador com endpoints mobile dedicados e paridade progressiva de fluxo.

## Historias

### IN_PROGRESS

- Nenhuma no momento.

### BACKLOG

- Nenhuma no momento.

### DONE

- `ST-001` - Fundacao do app do prestador (login, dashboard e pedidos proximos).
- `ST-002` - Detalhe de pedido e envio de proposta no app do prestador.
- `ST-003` - Agenda do prestador no app (pendencias e proximas visitas).
- `ST-004` - Chat realtime cliente x prestador no app do prestador.
- `ST-005` - Fluxo operacional de atendimento no app (chegada, inicio, checklist, evidencias).
- `ST-007` - Push Firebase no app do prestador com registro de token e deep link para conversa.
