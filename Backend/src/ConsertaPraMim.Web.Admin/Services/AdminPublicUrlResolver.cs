...
public static class AdminPublicUrlResolver
{
    ...
    public static string ResolveSwaggerUrl(
        string? candidateBaseUrl,
        string? requestHost,
        string fallbackUrl)
    {
        var apiBaseUrl = ResolveApiBaseUrl(candidateBaseUrl, requestHost, fallbackUrl);
        return $"{TrimTrailingSlash(apiBaseUrl)}/swagger";
    }
