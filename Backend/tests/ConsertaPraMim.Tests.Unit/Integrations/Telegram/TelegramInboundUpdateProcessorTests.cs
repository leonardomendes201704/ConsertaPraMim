...
public sealed class TelegramInboundUpdateProcessorTests
{
    ...
    [Fact(DisplayName = "Telegram inbound update processor | Nao deve voltar a pedir telefone quando ele ja estiver persistido")]
    public async Task ProcessAsync_NaoDeveVoltarAPedirTelefoneQuandoJaEstiverPersistido()
    {
        var update = new TelegramUpdate
        {
            UpdateId = 9012,
        ...
        var storedMessage = new ChatMessageDto(
            Id: "telegram:5513996891738:780",
            ChatId: 5513996891738,
            IsOutgoing: false,
            SenderDisplayName: "Mendes",
            Text: "eletricista",
            SentAtUtc: new DateTimeOffset(2026, 3, 16, 18, 26, 0, TimeSpan.Zero),
            Attachments: []);
        ...
        leadAutomationClient
            ...
            .ReturnsAsync(new TelegramLeadAutomationUpsertResult
            {
                ...
                HasServiceCategory = true,
                QualificationStatus = "dados_pendentes",
                MissingRequiredFields = ["Cidade", "CEP", "Logradouro ou bairro", "Contexto do problema"],
                Message = "Lead atualizado via automacao do bot Telegram."
            });
        automationClient
            .Setup(client => client.MirrorInboundMessageAsync(
        ...
        botApiClient
            .Setup(client => client.SendMessageAsync(
                ...
                It.Is<string>(text =>
                    text.Contains("sua cidade", StringComparison.Ordinal) &&
                    !text.Contains("seu telefone", StringComparison.Ordinal)),
                It.Is<IReadOnlyList<StoredLocalFile>>(files => files.Count == 0),
                It.IsAny<CancellationToken>(),
                It.Is<TelegramMessageSendOptions?>(options => options is not null && options.RemoveReplyKeyboard)))
            .Returns(Task.CompletedTask);
    ...
    [Fact(DisplayName = "Telegram inbound update processor | Nao deve voltar para categoria depois que ela ja estiver persistida")]
    public async Task ProcessAsync_NaoDeveVoltarParaCategoriaDepoisQueElaJaEstiverPersistida()
    {
        var update = new TelegramUpdate
        {
            UpdateId = 9013,
        ...
        var storedMessage = new ChatMessageDto(
            Id: "telegram:5513996891738:781",
            ChatId: 5513996891738,
            IsOutgoing: false,
            SenderDisplayName: "Mendes",
            Text: "11704150",
            SentAtUtc: new DateTimeOffset(2026, 3, 16, 18, 27, 0, TimeSpan.Zero),
            Attachments: []);
        ...
        leadAutomationClient
            ...
            .ReturnsAsync(new TelegramLeadAutomationUpsertResult
            {
                ...
                HasPostalCode = true,
                QualificationStatus = "dados_pendentes",
                MissingRequiredFields = ["Categoria", "Cidade", "Logradouro ou bairro", "Contexto do problema"],
                Message = "Lead atualizado via automacao do bot Telegram."
            });
        automationClient
            .Setup(client => client.MirrorInboundMessageAsync(
