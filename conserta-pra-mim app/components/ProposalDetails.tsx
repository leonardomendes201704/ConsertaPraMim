...
function resolveAppointmentStatusBadgeClasses(statusLabel: string): string {
  const normalized = statusLabel.trim().toLowerCase();
  ...
  if (normalized.includes('cancel') || normalized.includes('rejeit') || normalized.includes('expir')) {
    return 'bg-red-100 text-red-700 border-red-200';
  }
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-primary/10 bg-white p-4 shadow-sm space-y-3">
              <div className="flex items-center justify-between gap-2">
                ...
                {currentAppointment ? (
                  <span className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-bold ${resolveAppointmentStatusBadgeClasses(currentAppointment.statusLabel)}`}>
                    {currentAppointment.statusLabel}
                  </span>
                ) : null}
              </div>
              ...
              {currentAppointment ? (
                <div className="rounded-xl border border-primary/10 bg-primary/5 p-3">
                  <p className="text-xs font-bold text-[#2f6f6f]">Janela atual</p>
                  <p className="text-sm text-[#204646] font-semibold">{currentAppointment.windowLabel}</p>
                </div>
              ) : null}
