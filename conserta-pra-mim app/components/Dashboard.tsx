...
const CATEGORIES: ServiceCategory[] = [
  { id: '1', name: 'Elétrica', icon: 'bolt', color: 'bg-primary/10' },
  { id: '2', name: 'Hidráulica', icon: 'water_drop', color: 'bg-primary/10' },
    ...
    <div className="flex flex-col h-screen bg-background-light overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar pb-24">
        <section className="px-4 pt-6 pb-2">
          <h1 className="text-[#101818] text-3xl font-bold">Olá, João!</h1>
        ...
        <section className="px-4 py-4">
          ...
          {activeRequests.length === 0 ? (
            <div className="text-center py-8 bg-white rounded-xl border border-dashed border-primary/20">
              <p className="text-sm text-[#5e8d8d]">Você não tem pedidos ativos no momento.</p>
              <button onClick={onViewOrders} className="text-primary text-xs font-bold mt-2 hover:underline">Ver histórico completo</button>
            </div>
          ) : (
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-primary/60 font-medium">Pedido #{req.id}</p>
                    </div>
                  </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-primary/10 px-4 pb-4 pt-2 max-w-md mx-auto">
        <div className="flex items-center justify-between mb-2">
          <NavItem active icon="home" label="Início" />
