...
const OrdersList: React.FC<Props> = ({ requests, onBack, onShowDetails, onGoToHome, onGoToChat, onViewProfile }) => {
  const [activeTab, setActiveTab] = useState<'ATIVOS' | 'HISTORICO'>('ATIVOS');

  const activeOrders = requests.filter(r => r.status === 'AGUARDANDO' || r.status === 'EM_ANDAMENTO');
  const pastOrders = requests.filter(r => r.status === 'CONCLUIDO' || r.status === 'CANCELADO');

  const currentList = activeTab === 'ATIVOS' ? activeOrders : pastOrders;
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 pb-24">
        {currentList.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center opacity-40 py-20">
            ...
            <p className="text-sm">Seus pedidos aparecerão listados aqui.</p>
          </div>
        ) : (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`text-[10px] font-bold px-2 py-1 rounded-full uppercase ${
                    ...
                    req.status === 'CONCLUIDO' ? 'bg-green-100 text-green-600' :
